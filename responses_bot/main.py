...
bot = commands.Bot(
    ...
    intents=intents,
    description="custom responses bot by felix920506",
)
...
async def reload(ctx):
    if ctx.author.id == int(owner):
        ...
        print(
            clock()
            + f' user {ctx.author} issued "{ctx.message.content}" command but didn\'t have privileges'
        )
...
async def addresponse(ctx, trigger, response):
    ...
    if trigger in responses:
        if response in responses[trigger]:
            await ctx.send(
                f'response "{response}" for trigger "{trigger}" already exists, not adding anything'
            )
            print(
                clock()
                + f' response "{response}" for trigger "{trigger}" already exists, not adding anything'
...
async def deleteresponse(ctx, trigger, index=-1):
    ...
    if index == -1:
        ...
        print(
            clock()
            + f' response "{old_response}" to "{trigger}"" with index of {index} has been deleted.'
        )
        await ctx.send(
            f'response "{old_response}" with Trigger: {trigger} and index of {index} has been deleted.'
        )
...
async def editresponse(ctx, trigger, index, response):
    ...
    print(
        clock()
        + f' response with index of {index} to "{trigger}" has been changed from "{old_response}" to "{response}"'
    )
    await ctx.send(
        f'response with index of {index} to "{trigger}" has been changed from "{old_response}" to "{response}"'
    )
